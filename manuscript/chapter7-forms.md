...
1. If you haven't already got one, create a `forms.py` module within your Django apps's directory (`rango`) to store form-related classes.
...
I> - Ultimately, a HTTP `POST` may end up being programmed to create a new resource (e.g. a new database entry) on the server. This could later be accessed through a HTTP `GET` request.
